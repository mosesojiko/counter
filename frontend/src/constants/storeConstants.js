//create a store constants
export const CREATE_STORE_REQUEST = "CREATE_STORE_REQUEST";
export const CREATE_STORE_SUCCESS = "CREATE_STORE_SUCCESS";
export const CREATE_STORE_RESET = "CREATE_STORE_RESET";
export const CREATE_STORE_FAIL = "CREATE_STORE_FAIL";

//get store actions
export const GET_STORES_REQUEST = "GET_STORES_REQUEST";
export const GET_STORES_SUCCESS = "GET_STORES_SUCCESS";
export const GET_STORES_FAIL = "GET_STORES_FAIL";

//get single store constants, get store details
export const GET_SINGLE_STORE_REQUEST = "GET_SINGLE_STORE_REQUEST";
export const GET_SINGLE_STORE_SUCCESS = "GET_SINGLE_STORE_SUCCESS";
export const GET_SINGLE_STORE_FAIL = "GET_SINGLE_STORE_FAIL";


//get single store constants, get store details
export const GET_BIZ_STORE_REQUEST = "GET_BIZ_STORE_REQUEST";
export const GET_BIZ_STORE_SUCCESS = "GET_BIZ_STORE_SUCCESS";
export const GET_BIZ_STORE_FAIL = "GET_BIZ_STORE_FAIL";


//get user store
export const GET_USERSTORE_REQUEST = "GET_USERSTORE_REQUEST";
export const GET_USERSTORE_SUCCESS = "GET_USERSTORE_SUCCESS";
export const GET_USERSTORE_FAIL = "GET_USERSTORE_FAIL";

//edit store constants
export const Edit_STORE_REQUEST = "EDIT_STORE_REQUEST";
export const Edit_STORE_SUCCESS = "EDIT_STORE_SUCCESS";
export const Edit_STORE_FAIL = "EDIT_STORE_FAIL";
export const Edit_STORE_RESET = "EDIT_STORE_RESET";

//constants to update a store that is posted
export const POST_STORE_REQUEST = "POST_STORE_REQUEST";
export const POST_STORE_SUCCESS = "POST_STORE_SUCCESS";
export const POST_STORE_FAIL = "POST_STORE_FAIL";


//constants to update a store that is posted
export const UNPOST_STORE_REQUEST = "UNPOST_STORE_REQUEST";
export const UNPOST_STORE_SUCCESS = "UNPOST_STORE_SUCCESS";
export const UNPOST_STORE_FAIL = "UNPOST_STORE_FAIL";
